# -*- coding: utf-8 -*-
"""
脚本名称：4-labelme2yolo-1.py
功能描述：将LabelMe JSON格式的标签转换为YOLO格式，支持边界框和关键点转换
使用方法：修改dataset_root_folder为数据集根目录，直接运行脚本
依赖库：os, json, numpy
"""
import os
import json
...
# 数据集配置
dataset_root_folder = "./Triangle_labelme_25/"  # 数据集根目录
json_source = os.path.join(dataset_root_folder, "la")  # LabelMe JSON文件来源目录
yolo_target = os.path.join(dataset_root_folder, "yolo")  # YOLO格式标签输出目录
os.makedirs(yolo_target, exist_ok=True)  # 创建输出目录（如果不存在）
...
with open(yolo_txt_path, 'w', encoding='utf-8') as f:
    ...
    for each_ann in labelme['shapes']:  
        if each_ann['shape_type'] == 'rectangle':  # 处理边界框标注
            yolo_str = ''
