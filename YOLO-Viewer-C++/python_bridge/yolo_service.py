...
def _handle_inference(req: dict) -> dict:
    ...
    # Compute coordinate scale factors (C++ side resizes frame to max 640px)
    orig_w = float(req.get("orig_width", W))
    orig_h = float(req.get("orig_height", H))
    scale_x = orig_w / float(W) if W > 0 else 1.0
    scale_y = orig_h / float(H) if H > 0 else 1.0
    ...
    if task == "detect" and r.boxes is not None:
        ...
        detections = {
            "boxes": [[float(x1)*scale_x, float(y1)*scale_y, float(x2)*scale_x, float(y2)*scale_y] for (x1,y1,x2,y2) in boxes],
    ...
    # ── Pose ──
        ...
        for i in range(len(boxes)):
            ...
            kpts = [[float(kp[0])*scale_x, float(kp[1])*scale_y] for kp in kp_list]
