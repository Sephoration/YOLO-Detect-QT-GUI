...
bool YoloBridge::startService()
{
    ...
    // Resolve Python executable: try conda env, then PATH
    QString python;
    QStringList pythonCandidates;

    // 1. CONDA_PREFIX (activated conda environment)
    QString condaPrefix = qEnvironmentVariable("CONDA_PREFIX");
    if (!condaPrefix.isEmpty()) {
        pythonCandidates << condaPrefix + "/python.exe"
                         << condaPrefix + "/python"
                         << condaPrefix + "/bin/python";
    }

    // 2. Common conda environment paths (works without activation)
    QString home = QDir::homePath();
    QStringList condaBasePaths = {
        home + "/../A_ToolKit",              // D:/A_ToolKit
        home + "/Anaconda3",
        home + "/miniconda3",
        home + "/../anaconda3",
        home + "/../miniconda3",
        "C:/Users/" + QDir::home().dirName() + "/Anaconda3",
        "C:/Users/" + QDir::home().dirName() + "/miniconda3",
        "C:/Anaconda3",
        "C:/miniconda3",
    };
    for (const auto &base : condaBasePaths) {
        QString canonical = QDir(base).canonicalPath();
        if (canonical.isEmpty()) continue;
        // Prefer 'yolo' env if it exists, otherwise use base
        for (const auto &envName : QStringList{"yolo11", "yolo", ""}) {
            QString envPath = envName.isEmpty() ? canonical : canonical + "/envs/" + envName;
            if (QDir(envPath).exists()) {
                pythonCandidates << envPath + "/python.exe"
                                 << envPath + "/python"
                                 << envPath + "/bin/python";
            }
        }
    ...
    // 3. Default PATH (works when run from activated conda terminal)
    ...
    for (const auto &candidate : pythonCandidates) {
        ...
        // For bare names, check if they resolve via 'where'
        if (!candidate.contains('/') && !candidate.contains('\\')) {
            QProcess which;
            which.start("where", {candidate});
                                  ...
                                  double iouThreshold)
{
    ...
    obj["orig_width"]     = frame.cols;
    obj["orig_height"]    = frame.rows;
    sendJson(obj);
}

void YoloBridge::analyzeModel(const QString &modelPath)
{
    if (!isReady()) { emit serviceError("推理服务未就绪"); return; }
    ...
    obj["model_path"] = modelPath;
    sendJson(obj);
    // result arrives asynchronously via modelInfoReady signal
}
...
QJsonObject YoloBridge::matToJson(const cv::Mat &mat)
{
    ...
    // Resize to inference size first — drastically reduces transfer size
    cv::Mat small = resizeFrameForInference(mat, 640);

    std::vector<uchar> buf;
    cv::imencode(".jpg", small, buf, {cv::IMWRITE_JPEG_QUALITY, 50});
    QByteArray data(reinterpret_cast<const char*>(buf.data()), int(buf.size()));

    obj["format"]   = "jpg";
    obj["data"]     = QString::fromLatin1(data.toBase64());
    obj["width"]    = small.cols;
    obj["height"]   = small.rows;
    obj["channels"] = small.channels();
    return obj;
}
